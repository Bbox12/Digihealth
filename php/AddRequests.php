...
function myFunction() {
  ...
  if(myVarFromPhp==''){
window.location.replace("http://139.59.38.160/livestock/Dashboard/page-login.php");
  }else{
     if(error!=''){
      if(error==1){
           ...
           <td><?php echo $user['Name']; ?></td>
                    ...
                    <?php endif; ?>
                      ...
                      $conn = $db->connect();
                        $date=date("Y-m-d");
                         $server_ip="139.59.38.160";
                       $users =$conn->query("SELECT `ID`, `Name`, `isActive`, `User`, `Date`, `Time` FROM `requests` WHERE `isActive`=0");
                        if(!empty($users)): $count = 0; foreach($users as $user): $count++;
                    ?>
                    <tr>
                        <td><?php echo $count; ?></td>
                        ...
                        <td><a href="ActiveRequest.php?id=<?php echo $user['ID']; ?>" style="color: green;"><?php echo "Activate"; ?></td>
